...
class PaddleOcrEngine(private val context: Context) : OcrEngine {
    ...
    suspend fun recognizeBitmap(bitmap: Bitmap): String = withContext(Dispatchers.IO) {
        ensureInitialized()
        logDebug(TAG, "Bitmap: ${bitmap.width}x${bitmap.height}")

        val rawBoxes = detectText(bitmap)
        if (rawBoxes.isEmpty()) {
            logDebug(TAG, "No text boxes detected")
            return@withContext ""
        }
        val boxes = splitTallBoxes(rawBoxes)
        logDebug(TAG, "Detected ${rawBoxes.size} text boxes (${boxes.size} after split)")

        val rows = groupIntoRows(boxes)
        logDebug(TAG, "Grouped into ${rows.size} table rows")
    ...
    private fun ensureInitialized() {
        ...
        logDebug(TAG, "Detection model loaded")

        recSession = ortEnv.createSession(loadAsset("paddle/rec.onnx"))
        logDebug(TAG, "Recognition model loaded")

        dictionary = loadDictionary()
        logDebug(TAG, "Dictionary loaded: ${dictionary.size} characters")
    }
