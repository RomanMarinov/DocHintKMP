    ...
    suspend operator fun invoke(uri: Uri, fileType: FileType): String {
        ...
        when (TextCleaner.preFilter(cleanText)) {
            is TextCleaner.PreFilterResult.TooShort ->
                throw IllegalStateException("Слишком мало текста. Убедитесь, что документ — медицинский.")
            is TextCleaner.PreFilterResult.NoMedicalData ->
                throw IllegalStateException("Документ не содержит медицинских данных (анализов крови, дат, показателей).")
            is TextCleaner.PreFilterResult.Ok -> Unit
        }
